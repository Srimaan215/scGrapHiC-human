...
# Data paths - use existing preprocessed data from human_scGrapHiC
SCHIC_DIR = HUMAN_SCGRAPHIC_DIR / "GSE238001/pseudobulk_official"
CTCF_DIR = HUMAN_SCGRAPHIC_DIR / "annotations/preprocessed/ctcf"
CPG_DIR = HUMAN_SCGRAPHIC_DIR / "annotations/preprocessed/cpg"
RNA_DIR = HUMAN_SCGRAPHIC_DIR / "preprocessed/scrnaseq_pseudobulk"  # Updated to correct location
BULK_HIC_FILE = Path("/users/ssridh26/GM12878_ENCODE.hic")  # GM12878 bulk Hi-C from ENCODE
...
def process_cell_type(cell_type, output_path):
    ...
    for chrom in chromosomes:
        ...
        # Map cell type to directory name (handles different naming conventions)
        rna_cell_dir = CELL_TYPE_DIR_MAP.get(cell_type, cell_type)
        schic_cell_dir = cell_type  # scHi-C uses underscore format (B_NK, Unk_HSC, etc.)
        
        # File paths
        rna_file = RNA_DIR / rna_cell_dir / f"{chrom}_{PARAMETERS['resolution']}.npy"
        schic_file = SCHIC_DIR / schic_cell_dir / f"{chrom}.npy"
        ...
        metadatas[:, 2] = CELL_TYPE_MAP.get(cell_type, 28)  # cell type ID
...
def main():
    ...
    parser.add_argument('--cell_type', type=str, default='HSC',
    ...
    print("=" * 60)
    print(f"Creating NPZ for: {args.cell_type}")
    print("=" * 60)
    
    process_cell_type(args.cell_type, output_path)
